...
<template>
    <article v-for="(product, i) of productsList"
        class="relative group/blog-post rounded-lg overflow-hidden flex flex-col w-full h-screen mt-3 bg-default gap-2"
        :key="i">
            ...
            <UContainer class="flex flex-col items-baseline my-3">
                <UContainer class="flex flex-col items-baseline">
                    <div class="flex items-baseline w-full">
                        <h1 class="text-2xl text-pretty font-semibold text-highlighted">{{ capitalize(product.title) }}</h1>
                    </div>
                    <div class="flex flex-row justify-between items-center w-full my-3">
                        ...
                        <div class="flex flex-col items-baseline w-auto">
                            <span v-if="product.availability" class="flex flex-row items-center text-base h-7 gap-2">
                                ...
                                {{ capitalize($t('main.product.availability.' + product.availability.value)) }}: {{ product.stock }}
                            </span>
                        </div>
                    </div>
                </UContainer>
        ...
        <ProductOrganismsProductCollapsibleMarkdown :title="capitalize($t('main.product.collapsible.description'))" :description="product.description" />
        <ProductOrganismsProductCollapsibleMarkdown :title="capitalize($t('main.product.collapsible.ingredient'))" :description="product.ingredients" />
    </article>
</template>