...
<template>
    <UContainer class="flex flex-col items-center my-4">
        ...
        <UButton v-if="(!isProductInCart.value || isProductInCart.value) && isQuantityAvailable.value && product.stock > 0" color="neutral" variant="outline" size="xl"
            icon="fa6-solid:cart-shopping" @click.prevent="$emit('change', productNumber)"
            class="flex items-center w-auto h-auto min-w-48 text-xl p-4 border-2 border-solid border-(--color-text) rounded-lg relative" key="add-to-cart">
            {{ capitalize('ajouter au panier') }}</UButton>
        <UButton v-else color="neutral" variant="outline" size="xl" icon="fa6-solid:cart-shopping"
            class="flex items-center w-auto h-auto min-w-48 text-xl p-4 rounded-lg relative" key="not-available-to-cart" disabled>
